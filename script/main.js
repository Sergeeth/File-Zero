...
const modalShow = (modal) => {
    $(`.modal[modal=${modal}]`).fadeIn().css("display", "flex");
...
const modalHide = (modal) => {
    $(`.modal[modal=${modal}]`).fadeOut();
};

$(".modal").each(function () {
    const modal = $(this).attr("modal");
    const opentime = $(this).attr("opentime");
    const hidetime = $(this).attr("hidetime");
